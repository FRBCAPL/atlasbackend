...
const testLoginDirectly = async () => {
  try {
    ...
    console.log('\n🔍 Testing Login System Directly...\n');

    // Test specific users
    const testCases = [
      { email: 'frbcapl@gmail.com', pin: '777777', name: 'Mark Slam' },
      { email: 'kristyncristable@gmail.com', pin: '3468', name: 'Mark Test' },
      { email: 'randyfishburn@msn.com', pin: '5151', name: 'Randy Fishburn' },
      { email: 'rmeindl@aol.com', pin: '7485', name: 'Ryan Meindl' }
    ];
    ...
    console.log('\n🔍 Testing Actual Login Logic...');
    
    const testIdentifier = '3468'; // Mark Test's PIN
    console.log(`Testing login with identifier: ${testIdentifier}`);
    
    // Simulate the login logic from authController
    let user = await User.findOne({ 
      email: { $regex: new RegExp(`^${testIdentifier}$`, 'i') }
    });
      ...
      for (const potentialUser of allUsers) {
        try {
          if (potentialUser.pin && potentialUser.pin.length > 0) {
            const isPinMatch = await potentialUser.comparePin(testIdentifier);
            if (isPinMatch) {
              user = potentialUser;
    ...
    if (user) {
      ...
      console.log(`   ❌ No user found with identifier: ${testIdentifier}`);
    }
